...
function generateRollNumber(classNum, section, studentNum) {
    const year = new Date().getFullYear();
    const paddedNum = studentNum.toString().padStart(3, '0');
    return `${year}-${classNum}-${section}-${paddedNum}`;
}
function generateRealisticScore(maxScore) {
...
async function main() {
    ...
    for (const assignment of assignments) {
        ...
        const eligibleStudents = students.filter(s => s.class === assignment.class &&
